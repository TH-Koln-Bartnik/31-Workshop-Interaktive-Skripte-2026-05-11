...
[Memory-Spiel öffnen →](../assets/memory-spiel.html){target="_blank"}
:::
...
```
RStudio (schreiben)  →  GitHub Desktop (versionieren)  →  GitHub Pages (veröffentlichen)
        ↑                                                              │
        └──────────────────  AcademicCloud-Tutor (helfen)  ────────────┘
```
...
### Schritt 2 — Render und Preview

Im Terminal von RStudio (Tab unten):

```bash
quarto preview
```

Im Browser öffnet sich `http://localhost:4XXX` mit einer Live-Vorschau. Jede Änderung an einer `.qmd`-Datei führt zu sofortigem Reload.
...
### Schritt 4 — GitHub Desktop verknüpfen
